// patch_translations_pack_years.cjs
// Adds sPackYears to all 12 locale files.
// Idempotent.
//
// Run from project root:
//   node patch_translations_pack_years.cjs

const fs = require("fs");
const path = require("path");

const LOCALES_DIR = path.join("src", "locales");
const LANGS = [
  "no", "en", "nl", "fr", "de", "it", "sv", "da", "fi", "es", "pl", "pt",
];

const TRANSLATIONS = {
  no: "Pakkeår",
  en: "Pack-years",
  nl: "Pakjaren",
  fr: "Paquets-années",
  de: "Packungsjahre",
  it: "Pacchetti-anno",
  sv: "Paketår",
  da: "Pakkeår",
  fi: "Askivuodet",
  es: "Paquetes-año",
  pl: "Paczkolata",
  pt: "Maços-ano",
};

let totalAdded = 0;
let totalSkipped = 0;

for (const lang of LANGS) {
  const file = path.join(LOCALES_DIR, `${lang}.json`);
  if (!fs.existsSync(file)) {
    console.error(`✗ Missing locale file: ${file}`);
    continue;
  }

  const json = JSON.parse(fs.readFileSync(file, "utf8"));

  if (json.sPackYears !== undefined) {
    console.log(`  ${lang}.json — already has key, skipped`);
    totalSkipped++;
    continue;
  }

  json.sPackYears = TRANSLATIONS[lang];
  fs.writeFileSync(file, JSON.stringify(json, null, 2) + "\n", "utf8");
  console.log(`✓ ${lang}.json — added sPackYears`);
  totalAdded++;
}

console.log(`\nDone: ${totalAdded} file(s) updated, ${totalSkipped} already current.`);
