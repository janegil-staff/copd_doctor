...
Usage: python3 generate_copd_pdf.py <input.json> <output.pdf> [icon_path]
"""
...
# ─── CAT score colour ─────────────────────────────────────────────────────────
def cat_color(score):
    """CAT score (0-40) → row fill colour matching the app scale."""
    if score is None: return None
    if score <= 10:  return HexColor("#a8d5a2")  # green  — low       (0–10)
    if score <= 20:  return HexColor("#f5c97a")  # yellow — medium   (11–20)
    if score <= 30:  return HexColor("#f4a07a")  # orange — high     (21–30)
    return HexColor("#e87070")                   # red    — severe   (31–40)
...
def draw_human_silhouette(c, cx, base_y, h, w, col):
    ...
    p.curveTo(cx - sh_w*1.05, tor_top - tor_h*0.4,
              cx - hip_w*1.05, tor_top - tor_h*0.7, cx - hip_w, tor_bot)
    p.lineTo(cx + hip_w, tor_bot)
    p.curveTo(cx + hip_w*1.05, tor_top - tor_h*0.7,
              cx + sh_w*1.05,  tor_top - tor_h*0.4, cx + sh_w, tor_top)
    p.close(); c.drawPath(p, fill=1, stroke=0)
    ...
    for sign in [-1, 1]:
        ...
        p.curveTo(abx - sign * arm_w*0.6, arm_bot_y + arm_len*0.1,
                  atx - sign * arm_w*0.6, arm_top_y - arm_len*0.3,
                  atx - sign * arm_w*0.3, arm_top_y)
        p.close(); c.drawPath(p, fill=1, stroke=0)
    ...
    p.curveTo(cx - leg_gap*0.5, tor_bot - leg_h*0.2,
              lx + leg_w, tor_bot - leg_h*0.3, lx + leg_w*0.9, base_y)
    p.lineTo(lx, base_y)
    p.curveTo(lx, tor_bot - leg_h*0.4,
              cx - leg_gap*1.5, tor_bot - leg_h*0.1, cx - leg_gap*1.5, tor_bot)
    p.close(); c.drawPath(p, fill=1, stroke=0)
    ...
    p.moveTo(cx + leg_gap*0.5, tor_bot)
    p.curveTo(cx + leg_gap*0.5, tor_bot - leg_h*0.2,
              lx, tor_bot - leg_h*0.3, lx + leg_w*0.1, base_y)
...
def draw_bmi_figures(c, x, y, bmi):
    items = [
...
# ─── GENERATE ────────────────────────────────────────────────────────────────
def generate_pdf(data, out_path, icon_path=None):
    d = process(data)
    ...
    # Resolve medicine icon — prefer explicit path, fall back to script dir
    ...
    draw_bmi_figures(c, W - MR - 76, y - 2, d["bmi"])
    ...
    # ════════════════════════════════════════════════════════════════
    diag  = ("COPD (Confirmed by a physician)" if d["copd_confirmed"]
             else "COPD (Not confirmed by a physician)")
    ...
    c.drawString(ML, y, f"Alpha-1-antitrypsin deficiency screening: {alpha}"); y -= 21
    c.drawString(ML, y, f"Smoking status: {d['smoke_str']}"); y -= 21
    ...
    c.drawString(ML + 6, y - 10, f"Exacerbations past 12 months:  {total}  ({hosp_str})")
    y -= 21
    ...
    for idx, (yr, mo) in enumerate(months):
        ...
        # Second pass — day numbers + medicine icon
        ...
        for day in range(1, ndays + 1):
            ...
            # Medicine icon on Sunday of weeks where medicine was taken
    ...
    ROW_H = 18; BOX_SZ = 10
    COL_W = VACC_W - 4; BOX_X = vacc_x + COL_W - BOX_SZ
    vy = y - 20
    ...
    draw_score_table(c, ML,           score_y, f"GAD-7 score: {d['gad_total']}", gad_rows)
    draw_score_table(c, ML + each_w + 8, score_y, f"PHQ-9 score: {d['phq_total']}", phq_rows)
...
if __name__ == "__main__":
    ...
    with open(jp) as f:
        data = json.load(f)
    generate_pdf(data, op, icon_path=icon_arg)