...
export default function CopdSummaryPdfModal({ open, onClose, patient, t }) {
  ...
  const vaccCount = [vacc.flue, vacc.covid, vacc.pneumococ, vacc.rs, vacc.pertussis]
    .filter(Boolean).length;

  const smoking    = patient?.smoking ?? {};
  const smokeLabel = smoking.smoking === 1
  ...
  const previewItems = [
    ...
    { icon: "💊", label: t.medicines  ?? "Medications",       value: (patient?.userMedicines ?? []).length },
    { icon: "💉", label: t.vaccinations ?? "Vaccinations",   value: `${vaccCount}/5` },
    { icon: "🚬", label: t.smoking    ?? "Smoking",           value: smokeLabel },
    { icon: "🧠", label: "GAD-7 / PHQ-9",                    value: `${patient?.latestGad7 ? "✓" : "–"} / ${patient?.latestPhq9 ? "✓" : "–"}` },
  ];
            ...
            <div className="grid grid-cols-2 gap-2">
              {previewItems.map(({ icon, label, value }) => (
                <div
                  ...
                  style={{ background: "#f4f8f8", border: "1px solid rgba(38,142,134,0.1)" }}
                >
                  <span className="text-base leading-none">{icon}</span>
                  <div className="min-w-0">
                    <p className="text-xs truncate" style={{ color: "#7a9a98" }}>{label}</p>
                    <p className="text-sm font-semibold truncate" style={{ color: "#1a3a38" }}>
                      {value}
                    </p>
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            {loading
              ...
              : (t.downloadSummaryPdf ?? t.downloadPdf ?? "Download Summary PDF")}
          </button>
        </div>
      </div>
    </>
